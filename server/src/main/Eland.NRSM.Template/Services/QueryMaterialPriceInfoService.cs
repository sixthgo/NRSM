...
using Eland.NRSM.Template.QUERYMATERIALPRICEINFOV1IN;
...
namespace Eland.NRSM.Template.Services
{
    public class QueryMaterialPriceInfoService : GenericService<Domain.MaterialPriceInfo>, IQueryMaterialPriceInfoService
    {
        public Domain.MaterialPriceInfo GetMaterialPriceInforList(string werks, string matnr)
        {
            ...
            QueryMaterialPriceInfoV1InClient client = new QueryMaterialPriceInfoV1InClient();
            ...
            MaterialPriceInfoV1Query_sync param = new MaterialPriceInfoV1Query_sync();

            param.WERKS = werks;
            param.MATNR = matnr;

            MaterialPriceInfoV1Response_sync respone = client.MaterialPriceInfoV1QueryResponse_In(param);

            if (respone != null && respone.MaterialPriceV1List != null)
            {
                foreach (MaterialPriceV1List item in respone.MaterialPriceV1List)
                {
                    ...
                    dto.dLVTY = item.DLVTY;
                    dto.eINDT = item.EINDT;
                    dto.mENGE = item.MENGE;
